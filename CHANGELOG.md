...
### 时间轴与 Kirakara 渲染一致性 - 2026-09-25

- 新生成和注音确认后重新生成的时间轴统一前移 300 ms，同时保持个人复听提前量设置不变。
- 修正阴影版逐字遮罩越过字符边界时的裁切值，避免高亮阴影溢出或反向裁切。
- `.krl` 工程固定按 `config -> LRC 逐字正文 -> @RubyN` 排列，Ruby 标签从 `@Ruby1` 开始连续编号。
- 浏览器成片在等比放入原视频前显式绘制黑色画布，与云端 FFmpeg 的固定画布和黑边规则保持一致。
- 实时预览改用与本地导出相同的 Canvas 字幕绘制器，消除 DOM 预览与导出之间的字号、基线、描边和逐字裁切偏差。
