...
不得修改歌词表面文字。含英文或数字的范围只能在本地读音明显不符合日语语境时修正 reading，
不得改写 surface，也不得声称根据未提供的音频判断实际唱法。
        ...
        for raw_correction in response["corrections"]:
            ...
            for is_kanji, segment in _surface_segments(rebuilt_surface):
                if is_kanji:
                    pattern_parts.append("(.+?)")
                    continue
                for part in _FOREIGN_READING_PARTS.findall(segment):
                    pattern_parts.append(
                        "(.+?)"
                        if _LATIN_OR_DIGIT.search(part) is not None
                        else re.escape(_reading_for_literal_surface(part))
                    )
            pattern_parts.append("$")
            ...
            replacement = split_tokens_for_alignment(
                [
                    LyricToken(
