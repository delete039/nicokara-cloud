...
def integer_reading(digits: str) -> str:
    ...
    for group_index in range(len(groups) - 1, -1, -1):
        ...
        if unit == "ちょう":
            if group_reading.endswith("いち"):
                group_reading = group_reading[:-2] + "いっ"
            elif group_reading.endswith("はち"):
                group_reading = group_reading[:-2] + "はっ"
            elif group_reading.endswith("じゅう"):
                group_reading = group_reading[:-1] + "っ"
        parts.append(group_reading)
        parts.append(unit)
    return "".join(parts)


def _digit_by_digit_reading(digits: str) -> str:
    return "".join(_DIGIT_READINGS[int(character)] for character in digits)


def _safe_integer_reading(digits: str) -> str:
    try:
        return integer_reading(digits)
    except ValueError:
        return _digit_by_digit_reading(digits)
...
def _replace_last_digit_reading(
    digits: str,
    reading: str,
    replacements: dict[int, str],
) -> str:
    last = int(digits[-1])
    replacement = replacements.get(last)
    if replacement is None:
        return reading
    return _replace_ending(reading, _DIGIT_READINGS[last], replacement)


def _minute_parts(digits: str) -> tuple[str, str]:
    value = int(digits)
    base = _safe_integer_reading(digits)
    last = value % 10
...
def _day_parts(digits: str, *, follows_month: bool) -> tuple[str, str]:
    ...
    if special is None:
        reading = _safe_integer_reading(digits)
        if 11 <= value <= 31:
            reading = _replace_last_digit_reading(
                digits,
                reading,
                {7: "しち", 9: "く"},
            )
    ...
    *,
    ...
    if counter == "人":
        if value == 1:
            ...
            number_reading = _replace_last_digit_reading(
                digits,
                _safe_integer_reading(digits),
                {4: "よ", 7: "しち"},
            )
            counter_reading = "にん"
    elif counter == "分":
        ...
        if value > 0 and value % 10 == 0 and number_reading.endswith("じゅう"):
            number_reading = number_reading[:-1] + "っ"
        counter_reading = "ちょうめ"
    elif counter == "月":
        ...
        number_reading, counter_reading = _safe_integer_reading(digits), counter
...
def resolve_numeric_span(
    ...
    if following in _COUNTERS:
        follows_month = index >= 1 and surfaces[index - 1] == "月"
    ...
    reading = (
        ...
        else _digit_by_digit_reading(digits)
    )
    return NumericReadingSpan(
        consumed_items=1,
