...
| 运行保护 | 已完成 | 原子队列容量、持久限流、来源公平性、实时排队位置、任务取消、重启恢复和自动清理 |
...
- 单任务后台队列、原子容量预留、实时排队位置和任务取消，避免上传请求阻塞或并发超额入队
...
限流、顺序处理、排队位置和任务取消已经完成。下一阶段主要优化方向包括：

- 在真实多人上传场景中持续压测并调整限流参数
- 增加任务历史、用户隔离和存储配额
- 优化大文件上传和异常恢复能力
- 控制服务器资源占用，避免任务互相影响
...
NICOKARA_MAX_UPLOADS_PER_HOUR=6
...
- 后端保持单进程，不要增加 Uvicorn worker 数量；限流记录已持久化到 SQLite，但视频处理队列仍为单进程设计。
...
| `NICOKARA_MAX_UPLOADS_PER_HOUR` | `6` | 单个来源地址每小时最多创建的上传任务 |
...
当前测试基线为后端 103 项、前端 7 个测试文件共 33 项；前端覆盖 API 错误解析、服务器错误反馈、
...
- 后端应保持单进程，避免 SQLite、内存队列和限流状态不一致。
...
- 同一来源地址默认每小时最多创建 6 个上传任务，限流记录保存在 SQLite，服务重启不会清空。
- 默认每个来源最多同时拥有 2 个等待或处理中的任务，避免单个用户占满整个队列。
- Nginx 转发地址只在请求来自可信代理时使用，并从代理链右侧识别真实来源，防止伪造请求头绕过限流。
...
- 当前视频处理队列是单进程实现。部署多个后端实例前，需要将任务队列迁移到
  Redis/Celery 等共享服务；SQLite 持久限流可用于当前单服务器部署，但不适合作为高并发集群方案。
